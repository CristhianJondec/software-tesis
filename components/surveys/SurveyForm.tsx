...
import SurveyQuestionHelp from '@/components/surveys/SurveyQuestionHelp';
    ...
    return (
        <Tooltip.Provider delayDuration={200} skipDelayDuration={100}>
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                {instrument.questionHelp && (
                    <p className="-mb-2 text-sm text-[var(--text-secondary)]">
                        Si una expresión no te resulta clara, selecciona el ícono <CircleHelpLabel /> para ver una explicación breve.
                    </p>
                )}

                {instrument.questions.map((question, index) => {
                    const key = `item_${index + 1}`;
                    const help = instrument.questionHelp?.[index + 1];
                    return (
...
function CircleHelpLabel() {
    return (
        <span aria-hidden="true" className="inline-flex size-5 items-center justify-center rounded-full border border-[#663820] text-xs font-bold text-[#663820]">
            ?
        </span>
    );
}
